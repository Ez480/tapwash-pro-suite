...
begin
  ...
  v_name := coalesce(v_customer.full_name, v_profile.full_name, v_last.customer_name);
  v_phone := coalesce(v_customer.phone, v_profile.phone, v_last.customer_phone);
  v_email := coalesce(v_customer.email, v_profile.email, v_last.customer_email);
...
begin
  ...
  v_name := coalesce(v_customer.full_name, v_profile.full_name, v_last.customer_name);
  v_email := coalesce(v_customer.email, v_profile.email, v_last.customer_email);
  v_phone := coalesce(nullif(trim(p_phone),''), v_customer.phone, v_profile.phone, v_last.customer_phone);
