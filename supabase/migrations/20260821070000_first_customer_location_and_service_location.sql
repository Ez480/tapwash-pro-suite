...
begin
  ...
  if nullif(trim(coalesce(new.address, '')), '') is null and nullif(trim(coalesce(new.location_url, '')), '') is null then
    return new;
  end if;
...
begin
  if new.customer_id is null then return new; end if;
  if nullif(trim(coalesce(new.service_address, '')), '') is null and nullif(trim(coalesce(new.service_location_url, '')), '') is null then
    return new;
  end if;
  update public.profiles
