      ...
      <div className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
              <Badge variant={profile?.status === "active" ? "default" : "destructive"}>
                {profile?.status === "active" ? t("active") : t("suspended")}
              </Badge>
          ...
          <div className="ms-auto flex flex-wrap gap-2">
            <Dialog open={profileOpen} onOpenChange={setProfileOpen}>
              <DialogTrigger asChild>
                <Button variant="outline">
                  <UserCog className="me-1.5 size-4" />
                  {t("edit_profile")}
                </Button>
              </DialogTrigger>
              <DialogContent>
            ...
            <Dialog open={passOpen} onOpenChange={setPassOpen}>
              ...
              <DialogContent>
                ...
                <DialogFooter>
                  <Button onClick={changePassword} disabled={password.length < 6}>
                    {t("save")}
                  </Button>
                </DialogFooter>
              </DialogContent>
        ...
        {sub ? (
          <>
            <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <StatCard
                ...
                label={t("package")}
                value={
                  sub.packages ? pick(sub.packages.title_en, sub.packages.title_ar) : t("none")
                }
                hint={`${total} ${pick("washes", "غسلة")}`}
              />
              <StatCard
                tone="primary"
                label={t("remaining_washes")}
                value={remaining}
                hint={`${used} ${pick("used", "مستخدمة")}`}
              />
              <StatCard label={t("subscription_status")} value={statusLabel(sub.status)} />
              <StatCard label={t("last_wash")} value={fmtDate(lastWash)} />
            </div>

            <div className="panel mt-6 p-6">
              <div className="flex items-center justify-between text-sm">
                <span className="font-semibold">{t("used_washes")}</span>
                <span className="text-muted-foreground">
                  {used} / {total}
                </span>
              </div>
              <Progress value={total ? (used / total) * 100 : 0} className="mt-3" />
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                <div>
                  <p className="text-xs uppercase tracking-widest text-muted-foreground">
                    {t("start_date")}
                  </p>
                  <p className="mt-1 font-semibold">{fmtDate(sub.start_date)}</p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-widest text-muted-foreground">
                    {t("end_date")}
                  </p>
                  <p className="mt-1 font-semibold">{fmtDate(sub.end_date)}</p>
                </div>
              </div>
            </div>
          </>
          ...
          <div className="panel mt-6 p-8 text-center">
            <h3 className="text-xl font-bold">{t("no_subscription")}</h3>
            <p className="mt-2 text-sm text-muted-foreground">{t("no_subscription_d")}</p>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-3">
          <Button size="lg" onClick={renew}>
            <RefreshCw className="me-1.5 size-4" />
            {t("renew")}
          </Button>
          <Button size="lg" variant="outline" asChild disabled={!wa}>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-2">
          <section className="panel p-6">
            <h3 className="flex items-center gap-2 text-lg font-bold">
              <CreditCard className="size-4 text-primary" /> {t("my_cards")}
            </h3>
              ...
              {(cards ?? []).map((c) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{c.serial_number}</p>
                  </div>
                  <Badge variant="secondary">{fmtDate(c.activation_date)}</Badge>
                </div>
              ))}
            </div>
          </section>
          ...
          <section className="panel p-6">
            <h3 className="flex items-center gap-2 text-lg font-bold">
              <Droplets className="size-4 text-primary" /> {t("wash_history")}
            </h3>
            <div className="mt-4 space-y-3">
              {(washes ?? []).length === 0 && (
                <p className="text-sm text-muted-foreground">{t("empty")}</p>
              ...
              {(washes ?? []).map((w) => (
                ...
                >
                  <span className="font-medium">{fmtDate(w.washed_at)}</span>
                  <span className="text-muted-foreground">{w.branch ?? "—"}</span>
                </div>
              ))}
            </div>
          </section>
