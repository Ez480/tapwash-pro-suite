...
function MembershipGlassCard({ sub, total, remaining, used, statusLabel, pick, t }: { sub: any; total: number; remaining: number; used: number; statusLabel: (s?: string | null) => string; pick: (en: string, ar: string) => string; t: (key: string) => string }) {
  ...
  return <section className="panel relative mt-6 overflow-hidden p-0 shadow-xl">
    <div className="absolute -left-16 -top-20 size-48 rounded-full bg-primary/10 blur-2xl" />
    <div className="absolute -bottom-20 -right-16 size-52 rounded-full bg-cyan-400/10 blur-2xl" />
    <div className="relative p-5 sm:p-6">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <div className="rounded-2xl border border-white/35 bg-white/20 p-4 shadow-sm dark:border-white/10 dark:bg-white/[0.05]"><div className="mb-3 flex size-10 items-center justify-center rounded-xl bg-primary/10 text-primary"><Package className="size-5" /></div><p className="text-xs text-muted-foreground">{t("package")}</p><p className="mt-1 font-bold">{packageName}</p></div>
        <div className="rounded-2xl border border-white/35 bg-white/20 p-4 shadow-sm dark:border-white/10 dark:bg-white/[0.05]"><div className="mb-3 flex size-10 items-center justify-center rounded-xl bg-primary/10 text-primary"><Droplets className="size-5" /></div><p className="text-xs text-muted-foreground">{t("remaining_washes")}</p><p className="mt-1 text-2xl font-black">{remaining}</p><p className="text-xs text-muted-foreground">{used} {pick("used", "مستخدمة")} / {total}</p></div>
...
function CustomerDashboard() {
  ...
  const [currentOrder, setCurrentOrder] = useState<CurrentOrder | null>(null); const [nextSubscriptionWash, setNextSubscriptionWash] = useState<string | null>(null); const [employeeInfo, setEmployeeInfo] = useState<EmployeeInfo | null>(null); const [employeeLoading, setEmployeeLoading] = useState(false);
  ...
  const isAdmin = (roles ?? []).includes("admin"); const isEmployee = !isAdmin && (roles ?? []).includes("employee"); const total = sub?.total_washes ?? 0; const used = sub?.used_washes ?? 0; const remaining = Math.max(total - used, 0); const activeSubscription = String(sub?.status ?? "").toLowerCase() === "active"; const lastWash = activeSubscription ? (washes ?? []).find((wash: any) => wash.subscription_id === sub?.id)?.washed_at ?? null : null;
  ...
  useEffect(() => { if (!user?.id || !activeSubscription || !sub?.id) { setNextSubscriptionWash(null); return; } let mounted = true; const loadNextWash = async () => { const { data } = await (supabase as any).from("employee_tasks").select("scheduled_at,status").eq("customer_id", user.id).eq("subscription_id", sub.id).not("status", "in", "(completed,cancelled)").not("scheduled_at", "is", null).gte("scheduled_at", new Date().toISOString()).order("scheduled_at", { ascending: true }).limit(1).maybeSingle(); if (mounted) setNextSubscriptionWash(data?.scheduled_at ?? null); }; void loadNextWash(); const channel = supabase.channel(`subscription-washes-${sub.id}`).on("postgres_changes", { event: "*", schema: "public", table: "employee_tasks", filter: `subscription_id=eq.${sub.id}` }, () => void loadNextWash()).subscribe(); return () => { mounted = false; void supabase.removeChannel(channel); }; }, [user?.id, sub?.id, activeSubscription]);
