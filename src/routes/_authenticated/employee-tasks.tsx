...
function EmployeeTasks() {
  ...
  // Live sync: whenever the admin edits this employee's row (employee_id, card_number,
  // job_title, ...), refetch immediately instead of waiting for a page refresh/refocus.
  useEffect(() => {
    if (!user?.id) return;
    const channel = supabase
      .channel(`employee-self-${user.id}`)
      .on(
        "postgres_changes",
