  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div><h1 className="text-2xl font-bold">{pick("My orders & tasks", "أوردراتي ومهامي")}</h1><p className="text-sm text-muted-foreground">{pick("All orders assigned to you with customer and location details.", "كل الأوردرات المكلف بها مع بيانات العميل والموقع بالتفصيل.")}</p></div>
        <Badge variant="secondary">{pending.length} {pick("available orders", "أوردر متاح")}</Badge>
      </div>
        ...
        {mine.map((task) => (
          <article key={task.id} className="panel p-5">
            <div className="flex flex-wrap justify-between gap-3">
              <div><div className="flex items-center gap-2"><h2 className="text-lg font-bold">{task.title}</h2><Badge>{task.status}</Badge></div><p className="mt-1 text-sm text-muted-foreground">{task.wash_type}</p></div>
