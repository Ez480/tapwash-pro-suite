    ...
    <div>
      ...
      <Dialog open={Boolean(selectedTask)} onOpenChange={(open) => { if (!open) setSelectedTaskId(null); }}>
        <DialogContent className="max-h-[90vh] max-w-4xl overflow-y-auto">
          {selectedTask ? <><DialogHeader><DialogTitle className="flex flex-wrap items-center gap-2">{selectedTask.title || "تفاصيل الأوردر"}<Badge>{statusLabel(String(selectedTask.status ?? ""))}</Badge><Badge variant="outline">{deliveryLabel(String(selectedTask.delivery_status ?? ""))}</Badge></DialogTitle><DialogDescription>المتابعة الحية للأوردر والصور الخاصة به.</DialogDescription></DialogHeader>
            <div className="grid gap-3 rounded-2xl border bg-muted/20 p-4 sm:grid-cols-2 lg:grid-cols-3"><div><span className="text-xs text-muted-foreground">العميل</span><p className="font-semibold">{selectedTask.customer_name || "—"}</p></div><div><span className="text-xs text-muted-foreground">رقم الأوردر</span><p className="font-semibold">{selectedTask.serial_number ? `#${selectedTask.serial_number}` : selectedTask.id}</p></div><div><span className="text-xs text-muted-foreground">آخر تحديث</span><p className="font-semibold">{fmtDate(selectedTask.updated_at)}</p></div><div><span className="text-xs text-muted-foreground">حالة الغسيل</span><p className="font-semibold">{statusLabel(String(selectedTask.status ?? ""))}</p></div><div><span className="text-xs text-muted-foreground">حالة الدليفري</span><p className="font-semibold">{deliveryLabel(String(selectedTask.delivery_status ?? ""))}</p></div>{selectedTask.employee_id ? <div><span className="text-xs text-muted-foreground">الموظف</span><p className="font-semibold">{selectedTask.employee_id}</p></div> : null}{selectedTask.address ? <div className="sm:col-span-2 lg:col-span-3"><span className="text-xs text-muted-foreground">العنوان</span><p className="font-semibold">{selectedTask.address}</p></div> : null}{selectedTask.notes ? <div className="sm:col-span-2 lg:col-span-3"><span className="text-xs text-muted-foreground">ملاحظات</span><p className="font-semibold">{selectedTask.notes}</p></div> : null}</div>
            <div><div className="mb-3 flex items-center gap-2"><Camera className="size-5 text-primary" /><h4 className="font-bold">صور هذا الأوردر</h4><Badge variant="outline">{selectedPhotos.length}</Badge></div>{selectedPhotos.length === 0 ? <p className="rounded-xl border border-dashed p-6 text-center text-sm text-muted-foreground">لم يتم رفع صور لهذا الأوردر حتى الآن.</p> : null}{selectedPhotos.length > 0 ? <div className="grid gap-4 sm:grid-cols-2"><div><h5 className="mb-2 font-semibold">قبل الغسيل</h5><div className="grid gap-3">{beforePhotos.map((photo) => <img key={photo.id} src={photo.url} alt="قبل الغسيل" className="w-full rounded-xl border object-cover" />)}{beforePhotos.length === 0 ? <p className="text-sm text-muted-foreground">لا توجد صورة.</p> : null}</div></div><div><h5 className="mb-2 font-semibold">بعد التنظيف</h5><div className="grid gap-3">{afterPhotos.map((photo) => <img key={photo.id} src={photo.url} alt="بعد التنظيف" className="w-full rounded-xl border object-cover" />)}{afterPhotos.length === 0 ? <p className="text-sm text-muted-foreground">لا توجد صورة.</p> : null}</div></div></div> : null}</div>
          </> : null}
        </DialogContent>
      </Dialog>
