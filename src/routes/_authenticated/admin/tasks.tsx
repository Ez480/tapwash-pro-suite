...
import { CheckCircle2, ClipboardList, ExternalLink, MapPin, Plus, Trash2, CarFront } from "lucide-react";
...
function AdminTasks(){
 ...
 const availableOffers=offers.filter((o:any)=>o.status==="active"&&(!o.start_date||new Date(`${o.start_date}T00:00:00`)<=new Date())&&(!o.end_date||new Date(`${o.end_date}T23:59:59`)>=new Date()));
 ...
 return <div className="space-y-6">
  ...
  <section className="panel p-6"><div className="grid gap-4 md:grid-cols-2">
   ...
   {form.order_type==="subscription"?<>
    ...
    <div><Label>{pick("Available offers","العروض المتاحة")}</Label><select className="mt-2 h-10 w-full rounded-md border bg-background px-3" value={form.offer_id} onChange={e=>handleOffer(e.target.value)} disabled={!form.customer_id}><option value="">{pick("Choose offer","اختر العرض")}</option>{availableOffers.map((o:any)=><option key={o.id} value={o.id}>{pick(o.title_en,o.title_ar)}{o.new_price!=null?` — ${o.new_price} EGP`:""}</option>)}</select></div>
   ...
   <div><Label>{pick("Wash type","نوع الغسيل")}</Label><select className="mt-2 h-10 w-full rounded-md border bg-background px-3" value={form.wash_type} onChange={e=>set("wash_type",e.target.value)}><option value="car_wash">{pick("Car wash","غسيل سيارة")}</option><option value="interior">{pick("Interior","داخلي")}</option><option value="exterior">{pick("Exterior","خارجي")}</option><option value="full">{pick("Full","كامل")}</option></select></div>
