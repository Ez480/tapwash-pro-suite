...
function AdminTasks() {
  ...
  const { data: profiles = [], refetch: refetchProfiles } = useAdminTable("profiles", "*", "created_at");
  ...
  const createTask = async () => {
    if (!form.employee_id || !form.customer_id || !form.title || !form.location_text) {
      toast.error(pick("Fill employee, customer, task and location", "اكمل الموظف والعميل والمهمة والموقع"));
      return;
    }
    ...
    setForm({ employee_id: "", customer_id: "", subscription_id: "", title: "", wash_type: "car_wash", location_text: "", latitude: "", longitude: "", scheduled_at: "", notes: "" });
    refetch();
    refetchProfiles();
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">{pick("Assign washes with customer, subscription and location details.", "كلف الموظف بالغسيل مع بيانات العميل والاشتراك والموقع بالتفصيل.")}</p>
        </div>
      </div>
      ...
      <section className="panel p-6">
        ...
        <div className="mt-4 space-y-3">
          {tasks.map((task) => { const emp = profiles.find((p) => p.id === task.employee_id); return <div key={task.id} className="rounded-xl border border-border p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-bold">{task.title}</p><p className="text-sm text-muted-foreground">{emp?.full_name || emp?.email} · {task.customer_name} · {task.customer_phone || "—"}</p><p className="mt-1 flex items-center gap-1 text-sm"><MapPin className="size-4" />{task.location_text}</p><p className="mt-1 text-xs text-muted-foreground">{task.package_name || pick("No package", "بدون باقة")} · {task.remaining_washes ?? "—"} {pick("remaining", "متبقي")}</p></div><div className="flex gap-2"><Button size="sm" variant="outline" onClick={() => updateStatus(task.id, task.status === "completed" ? "pending" : "completed")}><CheckCircle2 className="me-1 size-4" />{task.status}</Button><Button size="sm" variant="destructive" onClick={() => removeTask(task.id)}><Trash2 className="size-4" /></Button></div></div><p className="mt-2 text-xs text-muted-foreground">{task.scheduled_at ? fmtDate(task.scheduled_at) : pick("No scheduled time", "بدون موعد")}{task.notes ? ` · ${task.notes}` : ""}</p></div>; })}
