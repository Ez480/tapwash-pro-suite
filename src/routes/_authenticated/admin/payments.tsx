  ...
  return (
    <div className="space-y-6">
          ...
          <table className="w-full min-w-[900px] text-sm">
            <thead>
              <tr className="border-b bg-muted/40 text-right">
                ...
                <th className="p-3">موعد الطلب</th>
            ...
            <tbody>
              {bookingRequests.filter((r: any) => ["awaiting_proof", "paid", "rejected"].includes(String(r.payment_status))).map((r: any) => (
                <tr key={`booking-payment-${r.id}`} className="border-b last:border-0 hover:bg-muted/20">
                  ...
                  <td className="p-3 font-semibold">{fmtMoney(Number(r.amount ?? 0))}</td>
                  <td className="p-3">{manualPaymentMethod(r.payment_method)}</td>
                  <td className="p-3">{r.scheduled_at ? fmtDate(String(r.scheduled_at)) : "—"}</td>
                  <td className="p-3">
                    <Badge variant={r.payment_status === "paid" ? "default" : r.payment_status === "rejected" ? "destructive" : "secondary"}>
                      {r.payment_status === "awaiting_proof" ? "في انتظار التأكيد" : r.payment_status === "paid" ? "مدفوع" : "مرفوض"}
                    </Badge>
                  </td>
                  <td className="p-3"><Badge variant="outline">{r.status || "—"}</Badge></td>
                  <td className="p-3">
                    {r.payment_status === "awaiting_proof" ? (
                      <div className="flex gap-2">
                        <Button size="sm" onClick={() => confirmManualPayment(r)}><CheckCircle2 className="me-1 size-4" />تأكيد</Button>
                        <Button size="sm" variant="destructive" onClick={() => rejectManualPayment(r)}><XCircle className="me-1 size-4" />رفض</Button>
                      </div>
                    ) : <span className="text-xs text-muted-foreground">—</span>}
                  </td>
                </tr>
              ))}
              {bookingRequests.filter((r: any) => ["awaiting_proof", "paid", "rejected"].includes(String(r.payment_status))).length === 0 && (
                <tr><td colSpan={7} className="p-8 text-center text-muted-foreground">لا توجد طلبات دفع يدوية.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <CrudTable
        ...
        rowActions={(row) => String(row["status"] ?? "") === "pending" ? <Button variant="outline" size="sm" onClick={() => activateSubscription(row)}><CheckCircle2 className="me-1 size-4" />تأكيد الدفع وتفعيل الاشتراك</Button> : null}
      />
    </div>
  );
}
