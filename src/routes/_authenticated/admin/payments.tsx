...
function AdminPayments() {
  ...
  const { data: subscriptions } = useAdminTable("subscriptions", "id, customer_id, package_id, status, start_date, end_date, total_washes, used_washes, request_id", "created_at");
  ...
  const confirmSubscriptionPayment = async (row: any) => {
    ...
    const { data: result, error: activationError } = await (supabase as any).rpc("confirm_subscription_request", { p_request_id: requestId, p_admin_id: auth.user.id });
    if (activationError) return void toast.error(activationError.message);

    const subscriptionId = result?.subscription_id ?? result?.id ?? null;
    const { error: paymentError } = await (supabase as any).from("payments").update({ status: "paid", paid_at: new Date().toISOString(), subscription_id: subscriptionId }).eq("id", payment.id).eq("status", "pending");
    if (paymentError) return void toast.error(paymentError.message);
    const { error: requestUpdateError } = await (supabase as any).from("subscription_requests").update({ payment_status: "paid", confirmed_at: new Date().toISOString(), confirmed_by: auth.user.id, updated_at: new Date().toISOString() }).eq("id", requestId).eq("status", "pending");
    if (requestUpdateError) return void toast.error(requestUpdateError.message);
