...
import { Nfc, Link2, RefreshCw } from "lucide-react";
    ...
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-3xl border border-cyan-300/40 bg-gradient-to-br from-cyan-500/15 via-background to-blue-500/10 p-6 shadow-[0_18px_60px_rgba(8,145,178,0.12)] backdrop-blur-xl">
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4"><div className="flex size-14 items-center justify-center rounded-2xl border border-cyan-300/40 bg-cyan-400/15 shadow-lg shadow-cyan-500/10"><Nfc className="size-8 text-cyan-600 dark:text-cyan-300" /></div><div><h2 className="text-xl font-bold">{pick("NFC Card Center", "مركز كروت NFC")}</h2><p className="text-sm text-muted-foreground">{pick("Manage cards, assignments and secure reassignment.", "إدارة الكروت وربطها وإعادة ربطها بشكل آمن.")}</p></div></div>
      ...
      <CrudTable table="nfc_cards" title={t("a_cards")} columns={[{ key: "serial_number", label: t("serial_number") },{ key: "card_type", label: t("card_type"), render: (r) => typeLabel(r["card_type"]) },{ key: "status", label: t("status"), render: (r) => <Badge variant={r["status"] === "assigned" ? "default" : "secondary"}>{r["status"] === "assigned" ? t("assigned") : r["status"] === "blocked" ? t("blocked") : t("available")}</Badge> },{ key: "customer_id", label: t("customer"), render: (r) => customerOptions.find((c) => c.value === String(r["customer_id"]))?.label ?? "—" },{ key: "activation_date", label: t("activation_date"), render: (r) => fmtDate(r["activation_date"] ? String(r["activation_date"]) : null) }]} fields={[{ name: "serial_number", label: t("serial_number") },{ name: "uid", label: t("uid") },{ name: "card_type", label: t("card_type"), type: "select", defaultValue: "card", options: [{ value: "card", label: t("card") },{ value: "sticker", label: t("sticker") },{ value: "keychain", label: t("keychain") }] },{ name: "status", label: t("status"), type: "select", defaultValue: "available", options: [{ value: "available", label: t("available") },{ value: "assigned", label: t("assigned") },{ value: "blocked", label: t("blocked") }] },{ name: "activation_date", label: t("activation_date"), type: "date" }]} />
    </div>
  );
}
