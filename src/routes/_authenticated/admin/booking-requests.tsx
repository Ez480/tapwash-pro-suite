...
import { ClipboardCheck, MapPin, Send, XCircle, CheckCircle2, LockKeyhole, CalendarDays } from "lucide-react";
...
function BookingRequests() {
  ...
  const visibleRequests = requests.filter((r: any) => r.status !== "scheduled");
  ...
  const confirmBooking = async (r: any) => {
    if (["confirmed", "assigned", "completed", "cancelled", "rejected", "scheduled"].includes(r.status)) return;
    try {
      const { error } = await (supabase as any).from("booking_requests").update({ status: "confirmed" }).eq("id", r.id).not("status", "in", "(completed,cancelled,rejected,scheduled)");
  ...
  const assign = async (r: any, employeeId: string) => {
    ...
    if (!emp || ["rejected", "cancelled", "scheduled"].includes(r.status)) return;
    if (!canAssign(r)) {
    ...
    try {
      ...
      const { error } = await (supabase as any).from("employee_tasks").insert({
        ...
        car_id: r.car_id ?? null, car_brand: r.car_brand ?? null, car_model: r.car_model ?? null,
        car_color: r.car_color ?? null, plate_number: r.plate_number ?? null
      });
  ...
  const scheduleBooking = async (r: any) => {
    if (!scheduleDate) {
      toast.error(pick("Choose the execution day first", "اختار يوم تنفيذ الأوردر أولاً"));
      return;
    }
    if (["scheduled", "assigned", "completed", "cancelled", "rejected"].includes(r.status)) return;
    try {
      const { data: authData } = await supabase.auth.getUser();
      const createdBy = authData.user?.id ?? null;
      if (!createdBy) {
        toast.error(pick("Your session has expired. Please sign in again.", "انتهت جلسة تسجيل الدخول، سجل الدخول مرة أخرى."));
        return;
      }
      ...
      let totalWashes: number | null = null;
      let usedWashes: number | null = null;
      let remainingWashes: number | null = null;

      if (r.package_id) {
        const { data: pkg, error: pkgError } = await (supabase as any).from("packages").select("title_ar,title_en,washes_count").eq("id", r.package_id).maybeSingle();
        if (pkgError) throw pkgError;
        packageName = pkg?.title_ar || pkg?.title_en || null;
        totalWashes = pkg?.washes_count ?? null;
        const { data: sub } = await (supabase as any).from("subscriptions").select("id,total_washes,used_washes,remaining_washes,car_id").eq("customer_id", r.customer_id).eq("package_id", r.package_id).eq("status", "active").order("created_at", { ascending: false }).limit(1).maybeSingle();
        if (sub) {
          subscriptionId = sub.id;
          totalWashes = sub.total_washes ?? totalWashes;
          usedWashes = sub.used_washes ?? 0;
          remainingWashes = sub.remaining_washes ?? Math.max(Number(totalWashes ?? 0) - Number(usedWashes ?? 0), 0);
        }
      }
      if (r.offer_id) {
        const { data: offer, error: offerError } = await (supabase as any).from("offers").select("title_ar,title_en").eq("id", r.offer_id).maybeSingle();
      ...
      const paymentLabel = r.payment_method === "cash" ? pick("Cash", "كاش") : r.payment_method === "smart_wallet" ? pick("Smart Wallet", "محفظة") : r.payment_method === "instapay" ? "InstaPay" : pick("Bank transfer", "تحويل بنكي");
      const orderTitle = `${r.wash_type === "car_wash" ? pick("Customer booking", "حجز عميل") : `${pick("Customer booking", "حجز عميل")} - ${r.wash_type}`} · ${amount.toFixed(2)} EGP · ${paymentLabel}`;
      const executionDate = new Date(`${scheduleDate}T12:00:00`);

      const { data: task, error: taskError } = await (supabase as any).from("employee_tasks").insert({
        ...
        scheduled_at: executionDate.toISOString(),
        notes: r.notes,
        status: "pending",
        ...
        car_color: r.car_color ?? null,
        plate_number: r.plate_number ?? null
      });
      if (taskError) throw taskError;

      const { error: updateError } = await (supabase as any).from("booking_requests").update({ status: "scheduled", scheduled_at: executionDate.toISOString() }).eq("id", r.id);
      if (updateError) {
        if (task?.id) await (supabase as any).from("employee_tasks").delete().eq("id", task.id);
        throw updateError;
      }

      toast.success(pick("Order moved to scheduled orders", "تم تحويل الأوردر إلى الطلبات المجدولة"));
      setSchedulingId(null);
      setScheduleDate("");
      refetch();
    } catch (error: any) {
      toast.error(error?.message || pick("Could not schedule the order", "تعذر تحويل الأوردر إلى الطلبات المجدولة"));
    }
  };

  const cancelBooking = async (r: any) => {
    if (["completed", "cancelled", "rejected", "scheduled"].includes(r.status)) return;
  ...
  const statusLabel = (status: string) => ({ confirmed: pick("Confirmed", "تم التأكيد"), assigned: pick("Assigned", "تم التكليف"), scheduled: pick("Scheduled", "مجدول"), cancelled: pick("Cancelled", "ملغي"), rejected: pick("Rejected", "مرفوض") } as any)[status] ?? status;
  ...
  return <div className="space-y-6">
    ...
    <div className="space-y-4">
      {visibleRequests.length === 0 && <div className="panel p-8 text-center text-muted-foreground">{pick("No booking requests yet.", "مفيش طلبات حجز حالياً.")}</div>}
      {visibleRequests.map((r: any) => <div key={r.id} className="panel p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-bold">{r.customer_name} · {r.customer_phone || "—"}</h2><p className="text-sm text-muted-foreground">{r.customer_email || "—"}</p><p className="mt-2 text-sm"><b>{pick("Requested on", "تاريخ ووقت الطلب")}:</b> {fmtDate(r.created_at)} · {new Date(r.created_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</p><p className="mt-1 font-semibold"><b>{pick("Wash appointment", "موعد الغسيل")}:</b> {fmtDate(r.scheduled_at)} · {new Date(r.scheduled_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</p></div><div className="flex gap-2"><Badge>{statusLabel(r.status)}</Badge><Badge variant="secondary">{payment(r)}</Badge></div></div><div className="mt-4 grid gap-3 text-sm md:grid-cols-2"><p><b>{pick("Car", "العربية")}:</b> {[r.car_type, r.car_brand, r.car_model, r.car_color, r.plate_number].filter(Boolean).join(" · ") || "—"}</p><p><b>{pick("Amount", "المبلغ")}:</b> {Number(r.amount ?? 0).toFixed(2)} EGP</p><p className="flex items-center gap-1"><MapPin className="size-4" /><b>{pick("Address", "العنوان")}:</b> {r.address || "—"}</p><p><b>{pick("Payment status", "حالة الدفع")}:</b> {r.payment_status}</p></div>{r.notes && <p className="mt-3 rounded-lg bg-muted p-3 text-sm">{r.notes}</p>}{isOnlinePayment(r) && r.payment_status !== "paid" && <div className="mt-3 flex items-center gap-2 rounded-lg border border-amber-300/50 bg-amber-100/30 p-3 text-sm"><LockKeyhole className="size-4 shrink-0" /><span>{r.payment_status === "rejected" ? pick("Payment rejected. Employee assignment is blocked until the payment issue is resolved.", "الدفع مرفوض. تكليف الموظف متوقف لحين حل حالة الدفع.") : pick("Employee assignment is blocked until the manager confirms or rejects this payment in Payments.", "تكليف الموظف متوقف لحين تأكيد أو رفض حالة الدفع من صفحة المدفوعات أولاً.")}</span></div>}<div className="mt-5 flex flex-wrap items-center gap-2">{!["confirmed", "assigned", "rejected", "cancelled", "completed", "scheduled"].includes(r.status) && <Button onClick={() => confirmBooking(r)}><CheckCircle2 className="me-1 size-4" />{pick("Confirm booking", "تأكيد الطلب")}</Button>}<select className="h-10 min-w-52 rounded-md border bg-background px-3 disabled:cursor-not-allowed disabled:opacity-50" defaultValue="" onChange={e => { if (e.target.value) assign(r, e.target.value); }} disabled={r.status === "assigned" || r.status === "rejected" || r.status === "cancelled" || r.status === "scheduled" || !canAssign(r)}><option value="">{r.status === "assigned" ? pick("Already assigned", "تم التكليف") : r.status === "cancelled" ? pick("Booking cancelled", "الحجز ملغي") : r.status === "rejected" ? pick("Booking rejected", "تم رفض الحجز") : !canAssign(r) ? pick("Resolve payment first", "احسم الدفع أولاً") : pick("Choose employee", "اختر الموظف")}</option>{employees.map((e: any) => <option key={e.id} value={e.id}>{e.full_name || e.email}</option>)}</select>{!["assigned", "rejected", "cancelled", "completed", "scheduled"].includes(r.status) && <Button variant="destructive" onClick={() => cancelBooking(r)}><XCircle className="me-1 size-4" />{pick("Cancel booking", "إلغاء الحجز")}</Button>}{r.status !== "assigned" && r.status !== "scheduled" && r.status !== "cancelled" && r.status !== "completed" && <><Button variant="outline" onClick={() => { setSchedulingId(schedulingId === r.id ? null : r.id); setScheduleDate(""); }}><CalendarDays className="me-1 size-4" />{pick("Move to scheduled orders", "تحويل إلى الطلبات المجدولة")}</Button>{schedulingId === r.id && <div className="flex items-center gap-2"><input type="date" value={scheduleDate} min={new Date().toISOString().slice(0, 10)} onChange={e => setScheduleDate(e.target.value)} className="h-10 rounded-md border bg-background px-3"/><Button onClick={() => scheduleBooking(r)} disabled={!scheduleDate}>{pick("Confirm day", "تأكيد اليوم")}</Button></div>}</>}{r.location_url && <Button variant="outline" onClick={() => window.open(r.location_url, "_blank", "noopener,noreferrer")}><MapPin className="me-1 size-4" />{pick("Open location", "فتح الموقع")}</Button>}<Button variant="outline" onClick={() => navigator.clipboard?.writeText(r.customer_phone || "")}><Send className="me-1 size-4" />{pick("Copy phone", "نسخ الرقم")}</Button></div></div>)}
    </div>
  </div>;
}
