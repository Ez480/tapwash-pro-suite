...
function AdminEmployees() {
  ...
  return <div>
    ...
    <Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-lg"><DialogHeader><DialogTitle>{editing ? t("edit") : t("add")}</DialogTitle></DialogHeader><div className="grid gap-4">{fields.map(([key,label,required,type]) => <div key={String(key)} className="space-y-2"><Label>{label}</Label><Input type={type} required={required} value={String(form[key] ?? "")} onChange={ev => setForm(cur => ({ ...cur, [key]: ev.target.value }))} /></div>)}<div className="space-y-2"><Label>{t("status")}</Label><Select value={String(form.status ?? "active")} onValueChange={v => setForm(cur => ({ ...cur, status: v }))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="active">{t("active")}</SelectItem><SelectItem value="inactive">{t("inactive")}</SelectItem></SelectContent></Select></div><p className="rounded-xl bg-muted p-3 text-xs text-muted-foreground">{pick("Only Employee ID, full name and job title are shown on the employee dashboard. Other fields remain management-only.", "الذي يظهر للموظف فقط هو رقم ID والاسم الكامل والمسمى الوظيفي. باقي البيانات خاصة بالمدير.")}</p></div><DialogFooter><Button variant="outline" onClick={() => setOpen(false)}>{t("cancel")}</Button><Button onClick={() => void save()} disabled={saving}>{saving ? pick("Saving…", "جاري الحفظ…") : t("save")}</Button></DialogFooter></DialogContent></Dialog>
  </div>;
}
