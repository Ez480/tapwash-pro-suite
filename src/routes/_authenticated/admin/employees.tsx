...
function AdminEmployees() {
  ...
  const save=async()=>{
    ...
    setSaving(true);
    try{
      const payload={employee_id:form.employee_id,national_id:form.national_id||null,card_number:form.card_number||null,full_name:form.full_name,email:form.email||null,phone:form.phone||null,job_title:form.job_title||null,branch:form.branch||null,status:form.status||"active"};
      const result=editing?await(supabase as any).from("employees").update(payload).eq("id",editing.id).select("*").single():await(supabase as any).from("employees").insert(payload).select("*").single();
      if(result.error)throw result.error;
      const userId=editing?.user_id;
      if(userId){const {error:profileError}=await supabase.from("profiles").update({full_name:payload.full_name,phone:payload.phone}).eq("id",userId);if(profileError)throw profileError;}
      toast.success(pick("Employee saved successfully","تم حفظ بيانات الموظف بنجاح"));setOpen(false);await queryClient.invalidateQueries({queryKey:["admin","employees"]});await refetch();
  ...
  const remove=async(employee:Employee)=>{if(!window.confirm(pick(`Delete ${employee.full_name}?`,`حذف الموظف ${employee.full_name}؟`)))return;const {error}=await(supabase as any).from("employees").delete().eq("id",employee.id);if(error)return void toast.error(error.message);toast.success(pick("Employee deleted","تم حذف الموظف"));await queryClient.invalidateQueries({queryKey:["admin","employees"]});await refetch()};
  return <div>
