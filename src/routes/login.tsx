  ...
  return (
    <div className="grid min-h-screen lg:grid-cols-2">
      <div className="flex flex-col p-6 sm:p-10">
        ...
        <div className="mx-auto flex w-full max-w-sm flex-1 flex-col justify-center py-12">
          <h1 className="animate-fade-up text-3xl font-bold">
            {mode === "in" ? t("login_title") : t("signup_title")}
          </h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {mode === "in" ? t("login_sub") : t("signup_sub")}
          </p>
          ...
          <Button variant="outline" size="lg" onClick={google} disabled={busy}>
            {t("google_sign_in")}
          </Button>
      ...
      <div className="relative hidden lg:block">
        <img
          src={heroCar}
          alt={pick("TapWash detailing studio", "استوديو تاب واش")}
          width={1600}
          height={1200}
        ...
        <div className="absolute bottom-12 start-12 end-12 text-ink-foreground">
          <p className="font-display text-3xl font-bold">{t("hero_badge")}</p>
          <p className="mt-2 max-w-md opacity-70">{t("cta_sub")}</p>
        </div>
      </div>
    </div>
  );
