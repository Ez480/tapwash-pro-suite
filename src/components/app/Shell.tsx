...
export function AdminNav({items}:{items:{to:string;label:string;icon:React.ElementType}[]}){const pathname=useRouterState({select:s=>s.location.pathname});const[pendingPayments,setPendingPayments]=useState(0);const[pendingBookings,setPendingBookings]=useState(0);const[unreadMessages,setUnreadMessages]=useState(0);useEffect(()=>{const load=async()=>{const[{count:paymentCount},{count:manualCount},{count:bookingCount},{count:messageCount}]=await Promise.all([supabase.from("payments").select("id",{count:"exact",head:true}).in("status",["pending","awaiting","unpaid"]),supabase.from("booking_requests").select("id",{count:"exact",head:true}).eq("payment_status","awaiting_proof"),supabase.from("booking_requests").select("id",{count:"exact",head:true}).in("status",["pending","new","awaiting"]),supabase.from("customer_messages").select("id",{count:"exact",head:true}).eq("is_read",false)]);setPendingPayments((paymentCount??0)+(manualCount??0));setPendingBookings(bookingCount??0);setUnreadMessages(messageCount??0);};void load();const ch=supabase.channel("admin-nav-badges").on("postgres_changes",{event:"*",schema:"public",table:"payments"},()=>void load()).on("postgres_changes",{event:"*",schema:"public",table:"booking_requests"},()=>void load()).on("postgres_changes",{event:"*",schema:"public",table:"customer_messages"},()=>void load()).subscribe();return()=>{void supabase.removeChannel(ch);};},[]);return <nav className="flex gap-1 overflow-x-auto pb-1">{items.map(i=>{const active=pathname===i.to;const isPayments=i.to==="/admin/payments";const isBookings=i.to==="/admin/booking-requests";const isMessages=i.to==="/admin/messages";const badgeCount=isPayments?pendingPayments:isBookings?pendingBookings:isMessages?unreadMessages:0;return <Link key={i.to} to={i.to} className={cn("flex shrink-0 items-center gap-2 rounded-xl px-3.5 py-2 text-sm font-medium transition-colors",active?"bg-primary text-primary-foreground shadow-card":"text-muted-foreground hover:bg-white/40 hover:text-foreground dark:hover:bg-white/10")}><span className={glassIcon}><i.icon className="size-4"/></span>{i.label}{badgeCount>0&&<span className={cn("inline-flex min-w-5 items-center justify-center rounded-full px-1.5 text-[10px] font-bold",active?"bg-primary-foreground text-primary":"bg-destructive text-destructive-foreground")}>{badgeCount>99?"99+":badgeCount}</span>}</Link>;})}</nav>;
}