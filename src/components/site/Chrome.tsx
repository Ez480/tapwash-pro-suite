...
export function ThemeToggle() {
  ...
  return <Button variant="ghost" size="icon" onClick={toggle} aria-label={t("theme")} className={cn("shrink-0 rounded-xl", glassIcon)}>{mode === "dark" ? <Sun className="size-4" /> : <Moon className="size-4" />}</Button>;
}
...
export function Header() {
  ...
  return <header className="sticky top-0 z-50 w-full min-w-0 overflow-x-clip border-b border-border/70 bg-background/70 shadow-sm backdrop-blur-2xl supports-[backdrop-filter]:bg-background/55"><div className="mx-auto flex h-16 w-full max-w-7xl min-w-0 items-center gap-2 px-3 sm:gap-4 sm:px-6"><Link to="/" className="flex min-w-0 shrink items-center gap-2"><span className="surface-blue flex size-10 shrink-0 items-center justify-center rounded-xl border border-white/45 shadow-[0_4px_14px_rgba(15,23,42,0.14)] dark:border-white/20"><span className="font-display text-xl font-black leading-none text-primary drop-shadow-[0_1px_3px_rgba(0,0,0,0.20)]">T</span></span><span className="flex min-w-0 flex-col leading-none"><span className="truncate font-display text-sm font-bold tracking-tight text-foreground sm:text-base">{settings ? pick(settings.company_name_en, settings.company_name_ar) : t("brand")}</span><span className="hidden text-[10px] font-medium uppercase tracking-widest text-muted-foreground sm:block">{t("tagline")}</span></span></Link><nav className="mx-auto hidden items-center gap-7 lg:flex">{nav}</nav><div className="ms-auto flex shrink-0 items-center gap-1.5 sm:gap-2 lg:ms-0"><LanguageToggle /><ThemeToggle /><Button asChild size="sm" className="hidden border border-white/30 shadow-sm sm:inline-flex"><Link to={user ? "/dashboard" : "/login"}>{user ? t("nav_dashboard") : t("nav_login")}</Link></Button><Sheet open={open} onOpenChange={setOpen}><SheetTrigger asChild><Button variant="ghost" size="icon" className={cn("lg:hidden", glassIcon)} aria-label="Menu"><Menu className="size-5" /></Button></SheetTrigger><SheetContent side="top" className="glass-card pt-12"><nav className="flex flex-col gap-5 text-lg">{nav}</nav><Button asChild className="mt-6 w-full"><Link to={user ? "/dashboard" : "/login"} onClick={() => setOpen(false)}>{user ? t("nav_dashboard") : t("nav_login")}</Link></Button></SheetContent></Sheet></div></div></header>;
}
...
export function SiteLayout({ children }: { children: React.ReactNode }) { return <div className="customer-site flex min-h-screen min-w-0 w-full flex-col overflow-x-clip bg-background text-foreground"><Header /><main className="min-w-0 w-full flex-1">{children}</main><Footer /></div>; }
export function PageHero({ eyebrow, title, subtitle }: { eyebrow?: string | undefined; title: string; subtitle?: string | null | undefined }) { return <section className="surface-hero w-full border-b border-border/70"><div className="mx-auto w-full max-w-7xl px-3 py-12 sm:px-6 md:py-24">{eyebrow && <p className="animate-fade-in text-xs font-semibold uppercase tracking-[0.25em] text-primary">{eyebrow}</p>}<h1 className="animate-fade-up mt-4 max-w-3xl text-4xl font-bold text-balance-tight text-foreground md:text-6xl">{title}</h1>{subtitle && <p className="animate-fade-up mt-5 max-w-2xl text-lg text-muted-foreground">{subtitle}</p>}</div></section>; }
