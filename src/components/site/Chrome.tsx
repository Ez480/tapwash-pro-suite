...
export function Header() {
  ...
  return <header className="sticky top-0 z-50 w-full min-w-0 overflow-x-clip border-b border-border/70 bg-background/70 shadow-sm backdrop-blur-2xl supports-[backdrop-filter]:bg-background/55"><div className="mx-auto flex h-16 w-full max-w-7xl min-w-0 items-center gap-2 px-3 sm:gap-4 sm:px-6"><Link to="/" aria-label={pick("Home", "الرئيسية")} className="flex min-w-0 shrink items-center gap-2"><span className="flex size-11 shrink-0 items-center justify-center rounded-xl bg-transparent p-0"><img src="/icons/tapwash-brand.svg" alt="TapWash" className="size-10 object-contain" /></span><span className="flex min-w-0 flex-col leading-none"><span className="truncate font-display text-sm font-bold tracking-tight text-foreground sm:text-base">{settings ? pick(settings.company_name_en, settings.company_name_ar) : t("brand")}</span><span className="hidden text-[10px] font-medium uppercase tracking-widest text-muted-foreground sm:block">{t("tagline")}</span></span></Link><nav className="mx-auto hidden items-center gap-7 lg:flex">{nav}</nav><div className="ms-auto flex shrink-0 items-center gap-1.5 sm:gap-2 lg:ms-0"><LanguageToggle /><ThemeToggle /><Button asChild size="sm" className="hidden border border-white/30 shadow-sm sm:inline-flex"><Link to={user ? "/dashboard" : "/login"}>{user ? t("nav_dashboard") : t("nav_login")}</Link></Button><Sheet open={open} onOpenChange={setOpen}><SheetTrigger asChild><Button variant="ghost" size="icon" className={cn("lg:hidden", glassIcon)} aria-label="Menu"><Menu className="size-5" /></Button></SheetTrigger><SheetContent side="top" className="glass-card pt-12"><nav className="flex flex-col gap-5 text-lg">{nav}</nav><Button asChild className="mt-6 w-full"><Link to={user ? "/dashboard" : "/login"} onClick={() => setOpen(false)}>{user ? t("nav_dashboard") : t("nav_login")}</Link></Button></SheetContent></Sheet></div></div></header>;
}
