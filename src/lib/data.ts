 ...
 *
 * IMPORTANT:
 * The employee dashboard reads the exact employee row
 * whose user_id equals the currently logged-in user's id.
 *
 * No RPC.
 * No email fallback.
 * No old get_my_employee function.
 */
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from("employees")
        .select(
          "id,user_id,employee_id,national_id,card_number,job_title,branch,full_name,email,phone,status,created_at,updated_at"
        )
        .eq("user_id", userId)
        .maybeSingle();

      if (error) {
        console.error("useMyEmployee query error:", error);
        throw error;
      }
