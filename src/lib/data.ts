...
export function useSettings() { return useQuery({ queryKey: ["settings"], queryFn: async () => { const { data, error } = await supabase.from("site_settings").select("*").eq("id", 1).single(); if (error) throw error; return data; } }); }
...
export function useMyCards(userId?: string | null) { return useQuery({ queryKey: ["my-cards", userId], enabled: !!userId, queryFn: async () => { const { data, error } = await supabase.from("nfc_cards").select("*").eq("customer_id", userId!); if (error) throw error; return (data ?? []).map((card) => ({ ...card, status: card.status === "assigned" ? "active" : card.status === "blocked" ? "blocked" : card.status })); } }); }
...
export function useMyEmployee(userId?: string | null) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) { console.error("Employee lookup failed:", error); throw error; }
