...
export function useMyEmployee(userId?: string | null) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const byUser = await supabase.from("employees").select(columns).eq("user_id", userId!).order("updated_at", { ascending: false }).limit(1).maybeSingle();
      ...
      if (email) {
        const byEmail = await supabase.from("employees").select(columns).ilike("email", email).order("updated_at", { ascending: false }).limit(1).maybeSingle();
        if (byEmail.error) throw byEmail.error;
        if (byEmail.data) return byEmail.data;
      }

      const rpc = await supabase.rpc("get_my_employee");
      if (!rpc.error) { const row = Array.isArray(rpc.data) ? rpc.data[0] : rpc.data; if (row) return row; }
      return null;
    },
  });
}
...
export function useAdminTable<T extends string>(table: T, select = "*", orderBy = "created_at") {
  return useQuery({ queryKey: ["admin", table], staleTime: 0, refetchOnWindowFocus: true, queryFn: async () => { if (table === "employees") { const { data, error } = await supabase.rpc("admin_list_employees"); if (error) throw error; return (data ?? []) as any[]; } const { data, error } = await supabase.from(table as any).select(select).order(orderBy, { ascending: false }); if (error) throw error; return (data ?? []) as any[]; } });
}
